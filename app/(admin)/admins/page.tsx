      ...
      cell: (admin: any) => (
        <div className="flex items-center gap-3 text-left">
          ...
          <div className="flex flex-col">
            <span className="font-semibold text-slate-800 tracking-tight text-sm">{admin.name}</span>
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm animate-in fade-in duration-300">
          <div className="bg-white w-full max-w-lg rounded-[2.5rem] shadow-2xl overflow-hidden animate-in zoom-in-95 duration-300">
            <div className="p-8 border-b border-slate-100 flex justify-between items-center bg-slate-50/50">
              <h2 className="text-xl font-black text-slate-800 tracking-tight text-left">Pengaturan Akses Admin</h2>
            ...
            <form onSubmit={handleSubmit} className="p-8 space-y-6 text-left">
              <div className="text-left">
                <label className="block text-[10px] font-bold text-slate-500 mb-2 font-sans">Nama lengkap</label>
              ...
              <div className="text-left font-sans">
                <label className="block text-[10px] font-bold text-slate-500 mb-2">Email login</label>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="text-left font-sans">
                  <label className="block text-[10px] font-bold text-slate-500 mb-2">Hak akses</label>
                  <SearchableSelect 
                    value={formData.role}
                ...
                <div className="text-left font-sans">
                  <label className="block text-[10px] font-bold text-slate-500 mb-2">Status akun</label>
                  <SearchableSelect 
      ...
      {isDetailOpen && selectedAdmin && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm animate-in fade-in duration-300">
          <div className="bg-white w-full max-w-xl rounded-[2.5rem] shadow-2xl overflow-hidden animate-in zoom-in-95 duration-300">
            <div className="p-8 border-b border-slate-100 flex justify-between items-center bg-slate-50/50">
              <h2 className="text-xl font-black text-slate-800 tracking-tight text-left">Profil Administrator</h2>
            ...
            <div className="p-8 space-y-8 text-left">
               <div className="flex items-center gap-6">
                 <div className="w-24 h-24 rounded-[2rem] bg-slate-900 text-white flex items-center justify-center text-4xl font-black shadow-2xl shadow-slate-900/30">
                   {selectedAdmin.name.charAt(0)}
                 </div>
                 <div className="text-left font-sans">
                   <h3 className="text-3xl font-black text-slate-800 tracking-tight leading-tight mb-1">{selectedAdmin.name}</h3>
                   <div className="flex items-center gap-3">
                       <span className={cn(
                         "px-3 py-1 rounded-xl text-[10px] font-black shadow-sm border",
               ...
               <div className="grid grid-cols-1 md:grid-cols-2 gap-6 font-sans">
                  <div className="bg-slate-50 p-6 rounded-3xl border border-slate-100 text-left">
                    <p className="text-[10px] font-black text-slate-400 mb-2 flex items-center gap-1.5"><Mail size={12}/> Email login</p>
                    <p className="text-sm font-black text-slate-700 tracking-tight">{selectedAdmin.email}</p>
                  </div>
                  <div className="bg-slate-50 p-6 rounded-3xl border border-slate-100">
                    <p className="text-[10px] font-black text-slate-400 mb-2 flex items-center gap-1.5"><Calendar size={12}/> Didaftarkan pada</p>
               ...
               <div className="bg-slate-900 p-8 rounded-[2rem] shadow-xl relative overflow-hidden group">
                  ...
                  <div className="relative z-10 text-left font-sans">
                    <p className="text-[10px] font-black text-amber-400 mb-4">Security & credentials</p>
                    <div className="flex items-center gap-4 text-white/60 mb-6">
                       ...
                       <div>
                          ...
                          <p className="text-[10px] font-bold text-white/40 tracking-widest">Argon2id Hash Protocol</p>
                       </div>
                    </div>
                    <div className="flex gap-2">
                       <button className="bg-white/10 text-white px-4 py-2 rounded-xl text-[10px] font-black border border-white/10 hover:bg-white/20 transition-all font-sans">Reset password</button>
                    </div>
                  </div>
               </div>

               <div className="flex gap-3 pt-2 font-sans">
                  <button onClick={() => { setIsDetailOpen(false); handleOpenModal(selectedAdmin); }} className="flex-1 py-4 bg-white border border-slate-200 rounded-2xl text-[10px] font-black uppercase tracking-widest text-slate-600 hover:bg-slate-50 transition-all">EDIT PROFIL</button>
                  <button onClick={() => setIsDetailOpen(false)} className="flex-1 py-4 bg-slate-900 text-white rounded-2xl text-[10px] font-black uppercase tracking-widest shadow-xl active:scale-95 transition-all">TUTUP</button>
               </div>
            </div>
          </div>
        </div>
